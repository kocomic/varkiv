...
Docker Compose でソースから永続ライブラリを構築する場合は、コピーして検証できる [Quickstart](docs/QUICKSTART.md) に従ってください。最初のイメージ公開後、実際のイメージ座標を使ったコンテナー導入手順を案内します。プレースホルダーの Registry アドレスはサポート対象の導入方法ではありません。
